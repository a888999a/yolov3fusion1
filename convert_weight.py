...
#                    (var_name_mess[1] == 'yolo-v3' and (var_name_mess[-2] in preserve_org_names)): continue
        if (var_name_mess[-1]  in ['Adam','Adam_1', 'Adam_2', 'Adam_3','global_step' ,'beta1_power','beta2_power','ExponentialMovingAverage']): continue                
...
with tf.name_scope('input'):
    input_rgb = tf.placeholder(dtype=tf.float32, shape=(1, 416, 416, 3), name='input_rgb')
    input_lwir = tf.placeholder(dtype=tf.float32, shape=(1, 416, 416, 3), name='input_lwir')    
