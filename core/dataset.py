...
class Dataset(object):
    ...
    def __next__(self):
        ...
        with tf.device('/cpu:0'):
            ...
            batch_image_rgb = np.zeros((self.batch_size, self.train_input_size, self.train_input_size, 3))
            batch_image_lwir = np.zeros((self.batch_size, self.train_input_size, self.train_input_size, 3))
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    image_rgb,image_lwir, bboxes = self.parse_annotation(annotation)
                    label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes = self.preprocess_true_boxes(bboxes)

                    batch_image_rgb[num, :, :, :] = image_rgb
                    batch_image_lwir[num, :, :, :] = image_lwir                    
                    ...
                    num += 1
                self.batch_count += 1
                return batch_image_rgb,batch_image_lwir, batch_label_sbbox, batch_label_mbbox, batch_label_lbbox, \
    ...
    def random_horizontal_flip(self, image_rgb,image_lwir, bboxes):

        if random.random() < 0.5:
            _, w, _ = image_rgb.shape
            image_rgb = image_rgb[:, ::-1, :]
            image_lwir = image_lwir[:, ::-1, :]
            bboxes[:, [0,2]] = w - bboxes[:, [2,0]]

        return image_rgb,image_lwir, bboxes

    def random_crop(self, image_rgb,image_lwir, bboxes):
        ...
        if random.random() < 0.5:
            ...
            image_rgb = image_rgb[crop_ymin : crop_ymax, crop_xmin : crop_xmax]
            image_lwir = image_lwir[crop_ymin : crop_ymax, crop_xmin : crop_xmax]
    ...
    def random_translate(self, image_rgb,image_lwir, bboxes):
        ...
        if random.random() < 0.5:
            ...
            image_lwir = cv2.warpAffine(image_lwir, M, (w, h))
